// This test demonstrates character output wrapping without using the
// C++ STL std::string type.  A simple wrapper class with similar
// functionality is created.  This can be used to workaround conflicts
// with the C++ library that may occur on Windows when linking code
// against other libraries, such as Java or Qt.
