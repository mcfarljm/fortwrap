...
* `ALLOCATABLE` or `POINTER` arguments (derived type pointers are
  supported as function return values)
...
#### Derived type pointer function return values

Wrapping functions that return a derived type as a pointer is
supported (wrapping functions that return a derived type by value is
not currently supported).  For example, the following function returns
a pointer to a derived type that is a component of another derived
type:
...
FortWrap generates a C++ function that returns a pointer to a new
instance of the proxy class (in the above example, a new instance of
the `Object` class).  An ownership flag within the proxy class is used
to track ownership, and by default objects that are created from
pointers returned from a function do not take ownership.  This ensures
that the underlying Fortran pointer data does not get deallocated
multiple times.

FortWrap adds an ownership API that can be used to modify this
behavior.  This is accessed via the `_acquire()` and `_disown()`
methods.  These methods are only added to proxy classes that wrap
derived types that do appear as function return values.  Initially,
objects created as function return values do not own the underlying
data.  This can be changed by calling the `_acquire()` method.  Taking
ownership makes it possible to clean up the data in the case that the
function being wrapped allocates the pointer that it returns.

A similar concept is used by Swig.  For example, the Swig python
interface provides access to a `thisown` member.  Note that if C++
code generated by FortWrap is re-wrapped by Swig to create a Python
interface, one could always set `thisown = True` (or use the
`%newobject` directive), as the FortWrap-generated C++ code always
returns a pointer to a new instance of the proxy class.  In other
words, it is safe for Python to clean up the C++ memory that is
created by the function returning an object pointer.  However, whether
the underlying Fortran memory is cleaned up is controlled through
FortWrap's ownership API (i.e. by calling `_acquire()`).

Examples are available under `tests/pointers`.
...
includes inheritance and polymorphism.  (Supporting these features
requires more assumptions about how the Fortran compiler generates
code, so they may be considered more fragile.)
